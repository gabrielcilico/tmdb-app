...
void main() {
  ...
  PopularStore store = PopularStore(moviesServiceMock);

  test('Should fetch popular movies', () async {
    when(() => moviesServiceMock.popularMovies(1)).thenAnswer((_) async => getMoviesPageable());
    expect(store.movies.length, 0);

    await store.fetchMovies();

    verify(() => moviesServiceMock.popularMovies(1)).called(1);
